...
internal class HomeAssistantSearcherImpl @Inject constructor(
    ...
    override fun discoveredInstanceFlow(): Flow<HomeAssistantInstance> {
        if (hasCollector.get()) {
            Timber.w("discoveredInstanceFlow already has an active collector; skipping new collector to avoid crash.")
            return emptyFlow()
        }
        ...
        return serviceFlow
            ...
            .onStart {
                if (hasCollector.get()) {
                    Timber.w("discoveredInstanceFlow already has an active collector; skipping start.")
                    return@onStart
                }
